...
const UsersTable = ({ users }) => {
  ...
  const handleNameSort = (type) => {
    let datas = [...sortedUsers]; 

    if (type === "asc") {
      datas.sort((a, b) => (a.firstName || '').localeCompare(b.firstName || ''));
    } else {
      datas.sort((a, b) => (b.firstName || '').localeCompare(a.firstName || ''));
    }

    setSortedUsers(datas);
  };
  ...
  const handleFilter = (city) => {
    ...
    if (city !== "all") {
      let filteredData = newData.filter((user) => {
        return user.address.city === city; 
      });
      setSortedUsers(filteredData);
    } else {
      setSortedUsers(newData);
    }
  }
  ...
  const handleChange = (e) => {
    ...
    let datas = [...sortedUsers];
    let searchResult = datas.filter((data) => {
    ...
    if (searchItem == "") {
      ...
      let datas = [...sortedUsers]; 
        let searchResult = datas.filter((data) => {
            ...
            return searchItem.toLowerCase() == data.fname.toLowerCase()
...
const handleReset = () => {
  ...
  return (
    ...
    <Form className='d-flex' onSubmit={handleSearch}>
        <Form.Control type="text" placeholder="Enter search" value={searchItem} onChange={handleChange} />
  ...
  <th>Full Name
  <Button variant='info' onClick={() => handleNameSort("asc")}>👆</Button> ||
                        <Button variant='info' onClick={() => handleNameSort("dsc")}>👇</Button>
  </th>
  <th>Age
  ...
  <th>City
    <div>
                         <Dropdown>
                            ...
                            <Dropdown.Menu>
                                <Dropdown.Item href="#/action-1" onClick={() => handleFilter("all")}>All</Dropdown.Item>
                                {
                                    cities.map((city) => {
                                        return (
                                            <Dropdown.Item href="#/action-1" onClick={() => handleFilter(city)}>{
                                                city
                                            }</Dropdown.Item>
                                        )
                                    })
